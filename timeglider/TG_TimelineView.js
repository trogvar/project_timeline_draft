  ...
  */
  ...
  registerTitles: function () {
	...
	buildTimelineMenu : function () {
		...
		var $menu_bt = $("<div class='timeglider-footer-button timeglider-list-bt'></div>").appendTo($bts);
	...
	setupPresentation: function() {
		...
		var title = "<h2 class='no-select' data-timeline_id='primary'>" + pres.title + "</h2>",
			...
			switch (modal_type) {
				...
				default:
					...
					templ_obj.extra_class = (templ_obj.image) ? "has-image":"no-image";
   					...
   					$modal = $.tmpl(me._templates.event_modal_small,templ_obj).appendTo(CONTAINER);
					...
					debug.log("space_above: ", space_above);
